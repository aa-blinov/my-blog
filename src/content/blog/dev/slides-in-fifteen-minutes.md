...
Кажется, никто не любит собирать слайды. Я точно не люблю. Это всегда выглядит примерно одинаково: содержание готово, выводы есть, данные собраны – и тут нужно всё это перенести в PowerPoint, выровнять, подобрать шрифт, сдвинуть таблицу на три пикселя влево. Час уходит на вёрстку.
...
Marp собирает PDF прямо из markdown-файла. Сегодня почти у всех базы знаний уже в Markdown – документация, внутренние вики, отчёты. Языковые модели тоже отдают результат в нём по умолчанию. А ещё они знают синтаксис Marp – можно попросить модель собрать структуру презентации и сразу получить готовый файл, который компилируется без правок. Это меняет процесс: ты работаешь только с содержанием.
...
Для сложных корпоративных презентаций с брендингом и анимациями – не сюда. Если нужно быстро и по делу – Marp закрывает задачу раньше, чем PowerPoint успевает загрузиться.
