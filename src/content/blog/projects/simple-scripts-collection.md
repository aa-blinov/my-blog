...
Stdlib в Python недооценивают. `pathlib`, `hashlib`, `csv`, `json`, `socket`, `http.server`, `sqlite3`, `zipfile` – это покрывает большую часть ежедневных задач без единого `pip install`. Скрипт можно скинуть коллеге, и он запустится сразу – не нужно объяснять, как создать виртуальное окружение.
