...
Самое интересное здесь – архитектура. Whisper на CPU работает не мгновенно: секунд тридцать, иногда больше, в зависимости от длины записи. Если запускать это прямо в обработчике сообщений, бот будет просто висеть всё это время. Поэтому транскрипция вынесена в отдельный воркер через Huey и Redis: бот принимает задачу, кладёт в очередь, воркер берёт и обрабатывает, бот забирает результат и отвечает.
